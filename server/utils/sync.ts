  ...
  const predicate = async (a: MuxAsset) => {
    ...
    try {
      if (a.asset_id) {
        ...
        strapi.log.warn('[mux-video-uploader] Unresolvable asset for sync:', JSON.stringify(a));
        return;
      }
    } catch (err: any) {
      if (err && err.type === 'not_found') {
        ...
        strapi.log.error('[mux-video-uploader] Error when attempting to get asset:', err);
      }
    }
